...
async function handleToolCalls(toolCalls) {
    ...
    switch(toolCall.name) {
        ...
        case 'job_applicator':
            promptMessage = 'I can help you apply to jobs automatically. First, upload your resume in the Job Finder tab, then you can use the "Apply with AI" button on any job listing.';
            tabBtns.forEach(btn => {
                if (btn.dataset.tab === 'jobs') {
                    btn.click();
                }
        ...
        case 'application_status':
            promptMessage = 'You can check the status of your job applications in the Applications tab.';
            tabBtns.forEach(btn => {
    ...
    return `
        <div class="job-card">
            ...
            <div class="job-footer">
                ...
                <button class="ai-apply-btn" onclick="applyWithAI(this)" data-job='${JSON.stringify(job)}'>
                    <i class="fas fa-robot"></i> Apply with AI
                </button>
            </div>
        </div>
    `;
...
async function loadApplications() {
    ...
    try {
        ...
        const data = await callApi(`${API_BASE_URL}/execute_tool`, 'POST', formData);
